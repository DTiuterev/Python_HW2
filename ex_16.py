#16. Задайте список из n чисел последовательности (1+1/n)^n и выведите на экран их сумму.
# Пример. Для n = 6: {1: 2, 2: 2,25, 3: 2,37, 4: 2,44, 5: 2,49, 6: 2,52}

n = int(input('Введите число n, я задам список из n чисел последовательности (1+1/n)^n и выведу на экран их сумму:\n '))
sequence = [round((1 + 1 / i) ** i, 2) for i in range(1, n + 1)]
print(f'Последовательность: {sequence}')
print(f'Сумма чисел последовательности: {sum(sequence)}')
